...
create policy "courses: published, owner, or admin read"
  on public.courses for select to authenticated
  using (is_published or (select private.can_manage_course(id)));
...
create policy "enrollments: students join published courses"
  ...
  with check (
    ...
    and exists (
      select 1
      from public.courses
      where id = course_id and is_published
    )
  );
