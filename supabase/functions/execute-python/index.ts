...
Deno.serve(async (request) => {
  ...
  try {
    ...
    console.error(
      "Python execution relay failed",
      error instanceof Error ? error.message : "Unknown error",
    );
    return json({ error: "Could not run the Python program." }, 500);
  }
});
...
async function parseRequest(request: Request): Promise<ExecuteRequest> {
  ...
  const source = requiredString(body.source, "Python source");
  const stdin = optionalString(body.stdin, "Program input");
  const encoder = new TextEncoder();
  if (encoder.encode(source).byteLength > maxSourceBytes) {
    throw new HttpError(400, "Python source must be 20 KB or less.");
  }
...
function runnerConfiguration(): { url: string; secret: string } {
  ...
  return {
    url: new URL("/v1/execute/python", baseUrl).toString(),
    secret,
  };
}
